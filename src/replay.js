...
import { BitStream } from './bit-reader'
            ...
            if (maskBits[i] & (1 << j)) {
                if (dd[index].flags & DT_BYTE) {
                    if (dd[index].flags & DT_SIGNED) {
                        ...
                        data[dd[index].name] = sign * bs.readBits(dd[index].bits - 1) / dd[index].divisor
                    } else {
                        data[dd[index].name] = bs.readBits(dd[index].bits) / dd[index].divisor
                    }
                ...
                } else if (dd[index].flags & DT_FLOAT
                    ...
                    data[dd[index].name] = bs.readBits(dd[index].bits) * (360 / (1 << dd[index].bits))
...
class FrameDataReader {
    ...
    static event(r, deltaDecoders) {
        ...
        if (bs._index % 8 > 0) {
            r.seek(Math.floor(bs._index / 8) + 1)
        } else {
            r.seek(bs._index / 8)
        }
    ...
    static serverInfo(r) {
        let info = {
            ...
            mapCrc:        r.i(),
            clientDllHash: r.arrx(16, TYPE_UB),
            maxPlayers:    r.ub(),
            playerIndex:   r.ub(),
            isDeathmatch:  r.ub(),
            gameDir:       r.str(),
    ...
    static updateUserInfo(r) {
        return {
            ...
            clientCdKeyHash: r.arrx(16, TYPE_UB)
        }
    }
    ...
    static clientData(r, deltaDecoders) {
        ...
        while (bs.readBits(1)) {
            bs._index += 6 // weapon index
            let weaponData = readDelta(bs, weaponDataDecoder)
        }
    ...
    static spawnBaseLine(r, deltaDecoders) {
        ...
        while (true) {
            ...
            if (entityType & 1) {
                if (entityIndex > 0 && entityIndex <= 32) {
            ...
            entities[entityIndex] = readDelta(bs, deltaDecoders[entityTypeString])
        }
    ...
    static killedMonster(r) {
        // unused
        return null
    }

    static foundSecret(r) {
        // unused
        return null
    }
    ...
    static packetEntities(r, deltaDecoders) {
        ...
        let entityStates = []
        let entityCount = bs.readBits(16)
        let entityNumber = 0
        while (true) {
    ...
    static deltaPacketEntities(r, deltaDecoders) {
        ...
        let entityCount = bs.readBits(16)
        bs._index += 8 // either updatemask or delta sequence number
        ...
        while (true) {
            ...
            entityStates[entityNumber] = readDelta(bs, deltaDecoders[entityType])
        }
        ...
        for (let i = 0; i < directories.length; ++i) {
            ...
            while (!isFinalMacroReached) {
                ...
                switch (macro.type) {
                    ...
                    default:
                        throw new Error([
                            `Unexpected macro type (${macro.type})`,
                            ` at adress ${Number(r.tell() - 9).toString(16)}`
                        ].join(''))
                    break
                }
    ...
    static parseIntoChunks(buffer) {
        ...
        class ReplayChunk {
            ...
            setData(data) {
                ...
                this.reader = new Reader(this.data.buffer)
            }
        }
        ...
        while (r.tell() < directoryEndOffset) {
            ...
            if (frame.type < 2 /* 0 or 1 */) {
                let serverInfo = frame.data
                    .find(msg => msg.type === FrameDataReader.SVC.SERVERINFO)
                if (serverInfo) {
                    currentMap = new ReplayMap(serverInfo.data.mapFileName)
                    maps.push(currentMap)
                }

                let resourceList = frame.data
                    .find(msg => msg.type === FrameDataReader.SVC.RESOURCELIST)
        ...
        while (true) {
            ...
            if (offset >= directoryEndOffset) {
                ...
                currentChunk.setData(r.arrx(lastFrameLength, TYPE_UB))
                r.seek(offset)
...
Replay.readHeader = readHeader
Replay.readDirectories = readDirectories
Replay.readFrame = readFrame
Replay.readFrameData = readFrameData