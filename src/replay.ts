    ...
    static parseIntoChunks(buffer: ArrayBuffer) {
        ...
        while (r.tell() < directoryEndOffset) {
            ...
            if (frame.type < 2 /* 0 or 1 */) {
                let serverInfo = frame.data
                    .find((msg: any) => msg.type === FrameDataReader.SVC.SERVERINFO)
                if (serverInfo) {
                ...
                let resourceList = frame.data
                    .find((msg: any) => msg.type === FrameDataReader.SVC.RESOURCELIST)
        ...
        while (true) {
            ...
            if (frame.type < 2) {
                    ...
                    if (message.type === FrameDataReader.SVC.SOUND
                     || message.type === FrameDataReader.SVC.SPAWNSTATICSOUND) {
                        let sound = currentMap.resources.sounds
                            .find((s: any) => s.index === message.data.soundIndex)
                        if (sound) {
                            sound.used = true
                        }
                ...
                let sound = currentMap.resources.sounds
                    .find((s: any) => s.name === frame.sound.sample)
                if (sound) {
                    sound.used = true
                }
            }
