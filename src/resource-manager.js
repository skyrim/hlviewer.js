...
module.exports = (function () {
    ...
    ResourceManager.prototype._onResourceProgress = function (event) {
        ...
        if (event.lengthComputable) {
            ...
            $('#loading').find('p.res' + id + ' .loading-resource-status').html(Math.round(percentComplete * 100) + '%');
        }
    };

    ResourceManager.prototype._onResourceLoadAll = function () {
        setTimeout(function () {
            $('#loading').fadeOut(1000, function () {
                $('#loading').find('p').remove();
            });
        }, 3000);
    };
