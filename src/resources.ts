...
const resizeTexture = (pixels: Uint8Array, width: number, height: number,
    ...
    return map.models
        .map(model => {
            ...
            geometry.vertices = model.vertices
                .map((vertex: any) => new Vec3(vertex[0], vertex[1], vertex[2]))
            ...
            let nf = geometry.faces.filter((face, i) => {
                let mat: any = material.materials[face.materialIndex]
